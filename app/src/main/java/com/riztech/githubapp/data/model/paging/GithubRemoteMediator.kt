...
class GithubRemoteMediator @Inject constructor(
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, User>): RemoteKeyEntity? {
        ...
        return state.pages.lastOrNull() { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { repo ->
                ...
                userDatabase.remoteKeyDao().remoteKeysRepoId(repo.id.toLong())
            }
    }
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, User>): MediatorResult {
        ...
        try {
            val apiResponse = githubApi.getSearchUser(apiQuery, page, state.config.pageSize)
            ...
            userDatabase.withTransaction {
                ...
                userDatabase.userDao().insertAll(entity)
            }
